...
public class Graph {
    ...
    public void recomputeWalkingEdges(double walkingDistance) {
        for(Stop target: stops){
            for(Stop other: stops){
                // if the target stop is different to the to stop and the walking distance is small enough, creating the edge between nodes
                if(target != other && target.distanceTo(other) <= walkingDistance){
                    ...
                    // adding the edge to the stops and the edges field.
                    target.addEdge(targetEdge);
                    other.addEdge(otherEdge);
    ...
    public void removeWalkingEdges() {
        // removing the edge in edges field if the edge has type of walking
        ...
        // remove the walking edges in each stop in stops field
        for(Stop stop: stops){
