...
public class Stop implements Comparable<Stop> {
    ...
    private Collection<Stop> neighbourNodes = new HashSet<>();
    ...
    public Collection<Stop> getNeighbourNodes(){
        return this.neighbourNodes;
    }


    /** add a new edge out of this stop  */
    public void addNeighbourNode(Stop stop){
        // make sure add the neighbours excluding itself
        if(stop.compareTo(this) == 0){
            return;
        }
        for(Stop node: neighbourNodes){
            if(node.compareTo(stop) == 0){
                return;
            }
        }
        neighbourNodes.add(stop);
    }
     ...
     */
    /*public void deleteEdges(String type) {
