...
public class Graph {
    ...
    public void recomputeWalkingEdges(double walkingDistance) {
        for(Stop target: stops){
            for(Stop other: stops){
                // if target is not other and the distance is within the range
                if(target != other && target.distanceTo(other) <= walkingDistance){
                    ...
                    // if the edge is not being added, add it to edges
                    // treat it as outEdge for target and inEdge for other
                    if(edgeNotExist(edges, targetEdge)){
                        target.addOutEdge(targetEdge);
                        other.addInEdge(targetEdge);
    ...
    public void removeWalkingEdges() {
        ...
        Collection<Edge> removeEdges = new HashSet<>();
        for(Edge edge: edges){// if the edge type of waling, remove it
        ...
        for(Stop stop: stops){
            // delete the edges from each stop
            stop.deleteEdges(Transport.WALKING);
        }
    }
    ...
    public boolean edgeNotExist(Collection<Edge> list, Edge edge){
        ...
        for(Edge connection: list){
            // if the edge stored in list has same from stop and to stop, the edge is being added
